...
contract TransparentUpgradeableProxy is ERC1967Proxy {
    ...
    function _governanceMembers() private view returns (address[] memory) {
        address[] memory providers = new address[](_isRootRevoked() ? 0 : 1);
        if (!_isRootRevoked()) providers[0] = _PROXY_VALIDATOR_CONTRACT;
        return GovernanceMembers.collect(_proxyGovernanceStorage().localOverlords, providers, bytes4(keccak256("getRootOverlords()")));
    }
