...
library GovernanceMembers {
        ...
        private view returns (uint256)
    {
        ...
        for (uint256 i; i < members.length; ++i) {
            ...
            if (!found) {
                if (count == result.length) revert TooManyMembers();
                result[count++] = member;
            }
        }
        return count;
    ...
    /// @dev Proxy roots and local controllers each retain their independent
    /// 64-member limit. Their union can contain 128 unique members; an upstream
    /// root update must not strand otherwise valid local proxy governance.
    function combineControllers(address[] memory local, address[] memory roots)
        internal view returns (address[] memory result)
    {
        result = new address[](2 * MAX_MEMBERS);
        uint256 count = append(result, 0, local);
        count = append(result, count, roots);
        assembly ("memory-safe") { mstore(result, count) }
        for (uint256 i = 1; i < count; ++i) {
            address item = result[i];
            uint256 j = i;
