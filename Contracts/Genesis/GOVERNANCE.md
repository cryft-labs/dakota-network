...
The current compressed development genesis is identified by `development-release.json` and `SHA256SUMS`. Earlier archives and historical compiler output are not interchangeable. Verify the archive and extracted JSON on every node. Development deployment is owner-authorized; production promotion remains subject to live acceptance and approval.

The documented runtime contract limit remains **32,768 bytes**. The refreshed genesis corrects the historical placement and uses `config.contractSizeLimit: 32768`, verified against the selected Besu release. A source-size check is not proof that a running node applies that setting. Test an ordinary contract creation on both sides of the boundary in an isolated Besu network before launch.

The validator build remains Solidity **0.8.19 / London / optimizer 200 runs**. Other public contracts use **0.8.37 / Osaka / optimizer 200 runs**. A private execution profile is a separate Paladin/Pente compatibility decision.
...
| `DakotaDelegation` | Retains owner signatures and adds conservative shared gas bounds and bounded return data. | This is delegated account logic, not a chain-administrator registry; adding an admin backdoor would change its authorization model. |
| `DakotaDelegationBeaconDispatcher` | Direct immutable v2 dispatch replaces the per-account proxy route; configuration getters identify the beacon/protocol. | Recover the implementation through the beacon/registry, not by introducing a second dispatcher owner. |
...
The fixed registry and facade addresses are protocol wiring, not hard-coded EOAs. The current development-release manifest records the management/bootstrap roots and validator identities. Private storage now has initialized, rotatable admin/service/forwarder roles. See the current implementation and sponsorship guides for application and funding changes beyond this governance review.
