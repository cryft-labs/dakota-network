"""Deploy reviewed v1.1.0 logic only; root retains exclusive proxy-upgrade authority."""
...
OLD=Web3.to_checksum_address('0xD78a10d70d4fDeb4294521e8e2fD895Afa7f0b3b')
...
class AllocationDeployment(Deployment):
    def __init__(self,workspace,execute=False):
        self.workspace=Path(workspace).resolve();self.execute=execute
        self.out=self.workspace/'outputs/inventory-allocation-20260917';self.out.mkdir(exist_ok=True)
        ...
        base=REPO/'Contracts/Accounts/allocation-artifacts/osaka/Accounts'/NAME
        old=REPO/'Contracts/Accounts/projects-inventory-artifacts/osaka/Accounts'/NAME
    ...
    def run(self):
        ...
        implementation=self.deploy(NAME,label='MomentInventoryTokenV110')
        self.reject('implementation_cannot_initialize',implementation.functions.initialize(DEPLOYER,DEPLOYER),sender=DEPLOYER)
        assert implementation.functions.implementationVersion().call()=='1.1.0'
...
if __name__=='__main__':
    ...
    args=parser.parse_args();AllocationDeployment(args.workspace,args.execute).run()
